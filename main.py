...
for x in mission_success: # Will get the amount of successful missions by finding all True entries
    if x == True: # if x: also works. This is adding to the count for each True in the misssion_success variable
        count += 1
...
success_rate = count / len(mission_success) * 100 # Acquire the percentage of the successful missions

print("Success rate: " + str(round(success_rate,2)) + "%") # Show user the success rate in a percentage

print("Missions launched before the year 2000: ")
for x in range(len(mission_names)): # Using the range of the mission names to allow use in case more data is added
